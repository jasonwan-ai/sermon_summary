...
def get_or_load_transcript(
    ...
    if transcript_path.exists():
        print("Step 1: Loading existing transcript", flush=True)
        transcript = load_transcript(transcript_path)
        print(f"✓ Loaded transcript from: {transcript_path}\n", flush=True)
        return transcript, transcription_times, api_times, total_tokens_sent, total_tokens_received
    
    # Step 1: First pass with tiny model
    print("Step 1: First pass transcription (tiny model)", flush=True)
    tiny_start = time.perf_counter()
    first_pass_transcript, timestamp_text = transcribe_wav_file(wav_path, model_size="tiny", language="id", clear_model_after=True)
    transcription_times["tiny"] = time.perf_counter() - tiny_start
    print(f"First pass transcription (tiny model): {transcription_times['tiny']:.2f} seconds\n", flush=True)
    
    # Step 2: Extract sermon timestamps using LLM
    print("Step 2: Extracting sermon boundaries", flush=True)
    timestamp_start = time.perf_counter()
    timestamp_response, ts_tokens_sent, ts_tokens_received = extract_timestamp(timestamp_text)
    ...
    print(f"Timestamp extraction API call: {api_times['timestamp']:.2f} seconds\n", flush=True)
    
    # Step 3: Crop WAV file
    print("Step 3: Cropping audio to sermon portion", flush=True)
    crop_start = time.perf_counter()
    run_ffmpeg_crop_wav(wav_path, cropped_wav_path, timestamp_response.start_time, timestamp_response.end_time)
    ...
    transcript, _ = transcribe_wav_file(cropped_wav_path, model_size="medium", clear_model_after=True)
    transcription_times["medium"] = time.perf_counter() - medium_start
    print(f"Second pass transcription (medium model): {transcription_times['medium']:.2f} seconds\n", flush=True)
...
def process_single_file(
    ...
    try:
        ...
        transcript, transcription_times, api_times, total_tokens_sent, total_tokens_received = get_or_load_transcript(
            wav_path, cropped_wav_path, transcript_path
        )
        ...
        total_transcription_time = transcription_times["tiny"] + transcription_times["medium"]
        ...
        print(f"  Transcription time: {total_transcription_time:.2f} seconds (tiny: {transcription_times['tiny']:.2f}s, medium: {transcription_times['medium']:.2f}s)", flush=True)
        print(f"  API call time: {total_api_time:.2f} seconds (timestamp: {api_times.get('timestamp', 0.0):.2f}s, summary: {api_times['summary']:.2f}s)", flush=True)
